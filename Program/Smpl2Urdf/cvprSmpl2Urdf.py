...
class smpl2Urdf(object):
    ...
    def __call__(self,savePath,shape=None):
        ...
        print(-jointsPos[0])
        np.savetxt(
            os.path.join(os.path.dirname(savePath),os.path.basename(savePath)[:-5]+'_trans.txt')
            , -jointsPos[0])
        # root_link.iners.append(urdf_utils.Inertial([0,0,0], [0,0,0], self.mass[0], self.iner))
        root_link.iners.append(urdf_utils.Inertial(-jointsPos[0], [0,0,0], self.mass[0], self.iner))
        # root_link.iners.append(urdf_utils.Inertial([0,0,0], [0,0,0], self.mass[0], self.iner))
        ...
        for key, i in enumerate(self.jointInUrdfIdx):
            if i == 12:
                ...
                # link.iners.append(urdf_utils.Inertial([0, 0, parentpos[2]-jointpos[2]],[0,0,0],mass,self.iner))
                ...
                joint = urdf_utils.Joint(name, 'spherical', self.parentname[key+1], name, jointpos-parentpos, [0,0,0], [1,0,0])
            ...
            if i == 15:
                ...
                link = urdf_utils.Link(name)
                # link.iners.append(urdf_utils.Inertial([0, (length+2*weigth)/2, 0],[0,0,0],mass,self.iner))
            ...
            if self.isCal[key+1]:
                ...
                if name ==  'lankle':
                    ...
                    link.writeFile(file)
                    joint = urdf_utils.Joint(name, 'fixed', self.parentname[key+1], name, jointpos-parentpos, [0,0,0], [1,0,0])
                    joint.writeFile(file)
                elif name == 'rwrist':
                    link = urdf_utils.Link(name)
