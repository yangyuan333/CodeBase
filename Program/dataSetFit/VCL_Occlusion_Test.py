    ...
    for dir_path in dirs_path:
        ...
        for frame_path in frames_dir:
            ...
            for frame in frames:
                ...
                v4, j4 = smpl_data(betas=torch.tensor(data['betas']), thetas=torch.tensor(data['pose']), trans=torch.tensor(trans.astype(np.float32)), scale=torch.tensor(1), gR=None, lsp=False)
                j4 = j4.squeeze(0).numpy()
                v4 = v4.squeeze(0).numpy()
                
                exmat = np.array(exmats[0])
                exmat[:3, 3] = exmat[:3, 3] / data['scale'][0]
                exmat[:3, :3] = np.dot(exmat[:3, :3], np.linalg.inv(RRR[:3, :3]))
                exmat[:3, 3] = exmat[:3, 3] - np.dot(exmat[:3, :3], RRR[:3, 3][:,None])[:,0]
                vs_2d = Camera_project(v4, exmat, np.array(inmats[0]))
                img = cv2.imread(R'E:\Human-Training-v3.2/00000.jpg')
                for v in vs_2d:
                    cv2.circle(img, (int(v[0]), int(v[1])),1,(0,0,255),3)
                vs_2d_1 = Camera_project(v3, exmat, np.array(inmats[0]))

                for v in vs_2d_1:
                    cv2.circle(img, (int(v[0]), int(v[1])),1,(255,0,0),3)
                cv2.imshow('1', img)
                cv2.waitKey(0)
                print(0)